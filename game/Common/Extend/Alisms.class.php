...
class Alisms { 
    ...
    public function __construct($sms_code) {
        // 配置参数 
        $this->accessKeyId = C('SMS_ACCESS_KEY_ID'); 
        $this->accessKeySecret = C('SMS_ACCESS_KEY_SECRET');
        $this->signName = C('SMS_SIGN_NAME'); 
        $this->templateCode = $sms_code; 
    } 
    ...
    public function send_verify($mobile, $verify_code) { 
        $params = array ( 
                ...
                'ParamString'=>$verify_code
        ); 
        // 计算签名并把签名结果加入请求参数 B5教程网 www.bcty365.com 
